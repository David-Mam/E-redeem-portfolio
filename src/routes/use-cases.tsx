    ...
    <section className="bg-white overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        ...
        <ScrollStaggerContainer staggerDelay={0.09} className="mt-12 grid gap-6 lg:grid-cols-3">
          {useCases.map((u) => (
            <ScrollStaggerItem key={u.title} distance={18}>
              <div className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)]">
                <span
                  className="grid h-14 w-14 place-items-center rounded-2xl transition-transform duration-200 group-hover:scale-105"
        ...
        <ScrollReveal
          direction="up"
          distance={18}
          className="mt-12 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-8 sm:p-10 transition-all hover:shadow-md"
        >
